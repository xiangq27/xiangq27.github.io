...
public class Node {
    ...
    private boolean matchShutdownCommand(String command) {
        ...
        //     shutdown port
        // Synopsis:
        //     Shutdown a transfer server listening on port <port> at local node.
        // Required arguments:
        ...
        try {
            int port = Integer.parseInt(args[1]);
            TCPSock server =
                this.tcpMan.getLocalSock(this.addr, port, TCPSock.State.LISTEN);
            if (server != null) {
                logOutput("shuting down server running at port = " + port);
                server.close();
            } else {
                logError("no server running at port = " + port);
            }
